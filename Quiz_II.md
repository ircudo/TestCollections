...
- Because a Map is not a true collection, its characteristics and behaviors are differ from the other collections like List or Set.
...
- Set contains values only whereas Map contains pairs of keys and values.
...
- It needs to be overridden if we want to compare objects based on their properties.

# Q05: How to iterate map?
We cannot directly iterate map, but this interface has two methods that gives view set of map. 
These methods are:

- Set<Map.Entry<K, V>> entrySet(): It is a method that returns a set having the map's entries. 
- Set<K> keySet(): This method returns a set that has the map's keys.
...
# Try a couple of LinkedList Exercises:
